...
def learning():
    ...
    minRange = 0    #minimum number of new classes arrive at each step
    maxRange = 0   #maximum number of new classes arrive at each step 
    ...
    for i in range(stepNum,len(order)):
        ...
        #save the dataset partition object (classes)
        print(datetime.datetime.now())
        print("saving dataset partition")
        save_object(classes, 'classes.pkl')
        print(datetime.datetime.now())
        print("completed")
        K.clear_session()
