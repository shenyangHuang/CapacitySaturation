...
def learning():
    ...
    initNum = 10         #10 base knowledge class
    ...
    (predictions, pre_score) = CNN_Training.class_evaluation(RL_model, classes[0], output_dim=startNum, lr=lr, batch_size=batch_size, verbose=False)
    ...
    search_dir =  search_director.search_director(N_workers, lr, epoch_limit, discrete_levels,
                    batch_size, patience=patience, data_augmentation=data_augmentation)
    ...
    for i in range(int(startNum/increment),int(100/increment)):
        ...
        (Vexist, RL_model) = CNN_Training.episodic_train(RL_model, classes[i*increment], valMemory, memoryBuffer, output_dim=((i+1)*increment),
        ...
        (candidate, val_accus) = search_dir.search_step(instructions, params, Vexist[1], classes[i*increment], (i+1)*increment, RAS=False, valMemory=valMemory, memoryBuffer=memoryBuffer)
        ...
        (score, RL_model) = CNN_Training.episodic_train(RL_model, classes[i*increment], valMemory, memoryBuffer, output_dim=((i+1)*increment),
                                                        lr=lr, batch_size=batch_size, epochs=epochs, patience=patience,
