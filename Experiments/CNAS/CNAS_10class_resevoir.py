...
def learning():
    ...
    (predictions, pre_score) = CNN_Training.class_evaluation(RL_model, classes[0], output_dim=startNum, lr=p_s["lr"], batch_size=p_s["batch_size"], verbose=False)
    '''
    ...
    for i in range(int(startNum/10),10):
        ...
        for k in range(i*10+1, (i+1)*10):
            datasets.sum_data(classes, k, i*10, percentage=1.0)
        ...
        (Vexist, RL_model) = CNN_Training.episodic_train(RL_model, classes[i*10], valMemory, memoryBuffer, output_dim=((i+1)*10),
                                                        lr=p_s["lr"], batch_size=p_s["batch_size"], epochs=p_s["epochs"], patience=p_s["patience"],
                                                        verbose=False, test=False, log_path=log_path, 
        ...
        (candidate, val_accus) = search_dir.search_step(instructions, params, Vexist[1], classes[i*10], (i+1)*10, RAS=False, valMemory=valMemory, memoryBuffer=memoryBuffer)
        ...
        (score, RL_model) = CNN_Training.episodic_train(RL_model, classes[i*10], valMemory, memoryBuffer, output_dim=((i+1)*10),
                                                        lr=p_s["lr"], batch_size=p_s["batch_size"], epochs=p_s["epochs"], patience=p_s["patience"],
