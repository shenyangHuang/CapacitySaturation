...
from Search_Controller.RL.WiderActor import WiderActor
from Search_Controller.RL.deeperActor import deeperActor
...
class RL_director:
    ...
    '''
    def __init__(self, num_input, wider_max_actions, deeper_max_actions, max_layers, fc_limit, entropy_penalty=0.01, lr=0.001):
    ...
    def state_gen(self, val_diff, architecture, sample_size, numNew):
        ...
        accu = [val_diff]
        arch = [0,0,int(numNew)]
        for layer in architecture:      #count number of conv and fc layer from the architecture
            if(layer.Ltype == 'conv'):
                arch[0] = arch[0] + 1
            if(layer.Ltype == 'fc'):
                arch[1] = arch[1] + 1
    ...
    def _state_encoding(self, accu, arch, max_size, sample_size):
        ...
        arch2 = decimal_encoding(arch[1], max_size)
        numNew = decimal_encoding(arch[2], max_size)        #put it in the same scale as arch
        x = accu + [arch1] + [arch2] + [numNew]
        states = x.copy()
    ...
    def _random_sample(self, architecture, num_wider, num_deeper):
        ...
        for i in range(num_deeper):
            (convIdx,fcIdx) = self.find_layers(architecture)
            '''
    ...
    '''
    def find_layers(self, architecture):
        fcIdx = []
        convIdx = []
